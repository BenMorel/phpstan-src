		...
		if ($kind === PreparedAssignTarget::KIND_VARIABLE) {
			...
			if (is_string($var->name)) {
				...
				$scope = $scope->assignVariable(
					...
					[],
					$write !== null ? $nodeScopeResolver->getVariableWriteMarkersToPlant($write) : [],
			...
			if ($varType->isArray()->yes() || !(new ObjectType(ArrayAccess::class))->isSuperTypeOf($varType)->yes()) {
				if ($var instanceof Variable && is_string($var->name)) {
					...
					$scope = $scope->assignVariable(
						...
						TrinaryLogic::createYes(),
						[],
						$write !== null ? [$write->getMarkerExpr()] : [],
						$write !== null ? $this->getArrayDimWriteMarkersToKill($nodeScopeResolver, $target, $write) : [],
					);
				} else {
